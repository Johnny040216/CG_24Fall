...
var Ikunhair = {
  ...
  build: () => {
    ...
    Ikunhair.texture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, Ikunhair.texture);
    ...
    //头发参数
     shape_data.height=Ikunhair.size * 4;
    ...
    Ikunhair.constructMatrix(
      mult( 
         mult(rotateZ(-30),rotateY(90),rotateX(30)),
         translate(+Ikunhair.size * 1, +Ikunhair.size * 1.8, -Ikunhair.size * 0.45)      //第一个参数变大上移  第二个参数变小下移  第三个参数变小后移
       ),
    ...
    Ikunhair.constructMatrix(
      mult(
         ...
         translate(-Ikunhair.size * 1, +Ikunhair.size * 1.8, -Ikunhair.size * 0.45)      
       ),
      hair_vertices.vertices
     );
     vertices = vertices.concat(hair_vertices.vertices);
    ...
    Ikunhair.constructMatrix(
      mult(
         mult(rotateZ(20),rotateY(-90),rotateX(0)),
         translate(-Ikunhair.size * 1, +Ikunhair.size * 1.8, -Ikunhair.size * 0.45)      //第一个参数变小左移  第二个参数变小下移  第三个参数变小后移
       ),
      hair_vertices.vertices
     );
     vertices = vertices.concat(hair_vertices.vertices);
    ...
    //保存顶点个数，用于drawArray。
    Ikunhair.vertexNum = vertices.length;

    //创建存放顶点坐标的buffer，并传入数据vertices。
    Ikunhair.vbo = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, Ikunhair.vbo);
    gl.bufferData(gl.ARRAY_BUFFER, flatten(vertices), gl.STATIC_DRAW);
    gl.bindBuffer(gl.ARRAY_BUFFER, null);

    //创建存放顶点法向量的buffer，并传入数据normals。
    Ikunhair.nbo = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, Ikunhair.nbo);
    gl.bufferData(gl.ARRAY_BUFFER, flatten(normals), gl.STATIC_DRAW);
    gl.bindBuffer(gl.ARRAY_BUFFER, null);

    //创建存放纹理坐标的buffer，并传入数据textures。
    Ikunhair.tbo = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, Ikunhair.tbo);
  ...
  updateTransformMatrix: function () {
    ...
    var RE = scalem(Ikunhair.size, Ikunhair.size, Ikunhair.size);
    var T = translate(
      Ikunhair.position[0],
      Ikunhair.position[1],
      Ikunhair.position[2]
    );
    var R = rotateY(Ikunhair.RotateAngle);
    Ikunhair.transformMatrix = mult(T, mult(R, RE));
  },
  rotateRight: function () {
    //整只鲲鲲向右转的函数。
    Ikunhair.RotateAngle += Ikunhair.ROTATE_STEP;
    Ikunhair.updateTransformMatrix();
    if (Ikunhair.onChange) {
      Ikunhair.onChange();
    }
  },
  rotateLeft: function () {
  ...
  walkForward: function () {
    ...
    //这里做了一个正交分解。
    Ikunhair.position[0] +=
      Ikunhair.FORWARD_STEP * -1 * Math.sin(radians(Ikunhair.RotateAngle));
    Ikunhair.position[2] +=
      Ikunhair.FORWARD_STEP * Math.cos(radians(Ikunhair.RotateAngle));
    Ikunhair.updateTransformMatrix();
    if (Ikunhair.onChange) {
      Ikunhair.onChange();
    }
  ...
  walkBackward: function () {
    ...
    //这里也做了一个正交分解。
    Ikunhair.position[0] +=
      Ikunhair.FORWARD_STEP * Math.sin(radians(Ikunhair.RotateAngle));
    Ikunhair.position[2] +=
      Ikunhair.FORWARD_STEP * -1 * Math.cos(radians(Ikunhair.RotateAngle));
    Ikunhair.updateTransformMatrix();
    if (Ikunhair.onChange) {
      Ikunhair.onChange();
    }
  ...
  shrink: function () {
    //整只鲲鲲缩小的函数
    Ikunhair.size -= Ikunhair.RESIZE_STEP;
    Ikunhair.updateTransformMatrix();
    if (Ikunhair.onChange) {
      Ikunhair.onChange();
    }
