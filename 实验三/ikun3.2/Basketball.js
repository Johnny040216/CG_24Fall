//Basketball的纹理二维坐标和顶点坐标的映射函数，这里默认使用图片的(0,0)，是一个黑色的像素。
//也就是说，如果传入这个函数，永远只能得到黑色。
Basketball.prototype.texture_empty = function (theta, fai, size) {
  return vec2(0, 0);
}
//
//hexiu的纹理二维坐标和顶点坐标的映射函数，需要注入到顶点坐标生成函数中(Geometry.js中的函数)，随顶点坐标一起作为返回值传出来。
Basketball.prototype.texture_coordinate = function (x, y, z, height, width) {
  if (z < 0) {
    return vec2(0, 0);
  }
  x = (x + width / 2.0) / width;
...
/*
  Basketball对象生成的时候，用于设置黑修对象的位置 
*/
Basketball.prototype.setLocation = function (a, b, c) {
  this.position = vec3(a, b, c);
  this.updateTransformMatrix();
};

/*
  Basketball对象初始化：顶点，纹理，法向量坐标
  build中根据黑修身体的不同部位，一次计算好以上的所有坐标，创建并且存入缓存区
*/

Basketball.prototype.build = function () {

  //通过dom从html中获取到image资源作为黑修的纹理
  var image = document.getElementById("ikunImage");
  //创建纹理映射并且绑定
  ...
  //通过调用Geometry.js中的generator函数计算好Basketball身体的顶点，纹理和法向量的坐标并存入对应数组
...
/*
  Basketball对象生成和变换的时候，用于更新黑修变换矩阵的函数
*/
Basketball.prototype.updateTransformMatrix = function () {
...
/*
  Basketball对象生成的时候，用于设置黑修的身体（左右耳朵）初始位置和角度的函数
*/
Basketball.prototype.constructMatrix = function (matrix, vertices) {
...
/*
  此函数根据Basketball对象的现有旋转角度和平移步长，计算出Basketball的正交位移量
  并通过html响应函数和更新变换矩阵函数
  实现Basketball前进的操作
*/
Basketball.prototype.walkForward = function () {
  this.position[0] +=
...
/*
  ...
  实现Basketball后退的操作
*/
Basketball.prototype.walkBackward = function () {
...
/*
  此函数根据Basketball的现有旋转角度和旋转步长，修改旋转角度
  并通过html响应函数和更新变换矩阵函数
  实现Basketball向左旋转的操作
*/
Basketball.prototype.rotateLeft = function () {
...
/*
  ...
  实现Basketball向右旋转的操作
*/
Basketball.prototype.rotateRight = function () {
  this.rotateAngle += this.ROTATE_STEP;
  this.updateTransformMatrix();
  if (this.onChange) {
    this.onChange();
...
/*
  此函数根据Basketball的size单位属性和缩放步长，修改size单位大小
  并通过html响应函数和更新变换矩阵函数
  实现Basketball整体变小的操作
*/
Basketball.prototype.shrink = function () {
...
/*
  ...
  实现Basketball整体变大的操作
*/
Basketball.prototype.expand = function () {
  this.size += this.RESIZE_STEP;
  this.updateTransformMatrix();
  if (this.onChange) {
    this.onChange();
